{{
    config(
        ...
        post_hook='CREATE OR REPLACE TABLE `rj-cor.clima_satelite_staging.infravermelho_longo_banda_13_goes_16_last_partition` AS (SELECT CURRENT_DATETIME("America/Sao_Paulo") AS data_particao)'
    )
}}
...
SELECT
    ...
    SAFE_CAST(cmi AS FLOAT64) cmi,
    SAFE_CAST(horario AS TIME) horario,
    SAFE_CAST(DATE_TRUNC(DATE(data_particao), day) AS DATE) data_particao,
FROM `rj-cor.clima_satelite_staging.infravermelho_longo_banda_13_goes_16`
...
{% set max_partition = run_query(
    ...
    FROM `rj-cor.clima_satelite_staging.infravermelho_longo_banda_13_goes_16_last_partition`
    ").columns[0].values()[0] %}
