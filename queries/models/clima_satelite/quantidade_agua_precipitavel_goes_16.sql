{{
    config(
        ...
        post_hook='CREATE OR REPLACE TABLE `rj-cor.clima_satelite_staging.quantidade_agua_precipitavel_goes_16_last_partition` AS (SELECT CURRENT_DATETIME("America/Sao_Paulo") AS data_particao)'
    )
}}
...
FROM `rj-cor.clima_satelite_staging.quantidade_agua_precipitavel_goes_16`
...
{% set max_partition = run_query(
    ...
    FROM `rj-cor.clima_satelite_staging.quantidade_agua_precipitavel_goes_16_last_partition`
    ").columns[0].values()[0] %}
